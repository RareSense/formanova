...
function extractJewelryDescription(result: PhotoshootResultResponse): string | null {
  const preferredKeys = ['generate', 'output', 'generate_image', 'prepare'];
  const orderedKeys = [
    ...preferredKeys.filter(k => k in result),
    ...Object.keys(result).filter(k => !preferredKeys.includes(k)),
  ];
  for (const key of orderedKeys) {
    ...
    for (const item of items) {
      if (item && typeof item === 'object' && !Array.isArray(item)) {
        ...
        const desc = r.description ?? r.jewelry_description;
  ...
  useEffect(() => {
    ...
    for (const gen of running) {
      ...
      pollWorkflow<PhotoshootResultResponse>({
        ...
        const jewelryDescription = gen.isProductShot ? (extractJewelryDescription(result) ?? undefined) : undefined;
        ...
        setGenerations(prev => prev.map(g =>
          g.workflowId === gen.workflowId
            ? { ...g, status: 'completed', progress: 100, resultImages, jewelryDescription }
            : g
        ));
        markGenerationCompleted(gen.workflowId, startTime);
        refreshCredits();
