...
describe('ring_cad_nurbs_v1 start body', () => {
  ...
  it('defaults to Claude Opus 5.5 through OpenRouter', () => {
    const { payload } = buildRingCadStartBody({ referenceImages: [IMG(1)] });
    expect(payload.llm_tier).toBe(RING_CAD_DEFAULT_TIER);
    expect(RING_CAD_DEFAULT_TIER).toBe('claude_opus_5_5_openrouter');
  });
...
describe('jewelry_cad_generate', () => {
  it('new models are generated by the jewelry workflow', async () => {
    const { RING_CAD_WORKFLOW } = await import('./ring-cad-nurbs-api');
    expect(RING_CAD_WORKFLOW).toBe('jewelry_cad_generate');
  });

  it('sends jewelry_type, ring unless the caller says otherwise', () => {
    expect(buildRingCadStartBody({ referenceImages: [IMG(1)] }).payload.jewelry_type).toBe('ring');
    expect(
      buildRingCadStartBody({ referenceImages: [], userDescription: 'pendant', jewelryType: 'necklace' }).payload.jewelry_type,
    ).toBe('necklace');
  });
});
